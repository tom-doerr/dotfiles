...
# Ensure Colemak-DH move bindings exist
...
# Ensure no conflicting border binding on Shift+n
if rg -n '^bindsym \$mod\+Shift\+n +border normal$' "$cfg"; then
  echo "FAIL: duplicate binding: $mod+Shift+n also mapped to border normal" >&2
  exit 1
fi
